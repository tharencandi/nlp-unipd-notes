...
- **Observed Agreement**:
    $P_o = \sum_i{P(A=B=y_i)}=\frac{\sum_i{count(A=B=y_i)}}{total}$

    $P_o = \sum_i P(A = B = y_i) = \frac{\sum_i \text{count}(A = B = y_i)}{\text{total annotations}}$
- **Expected Agreement**:
	$P_e =  \sum_i{P(A=y_i)P(B=y_i)}$
- **Cohen’s Kappa**:
...
### Variants
- Pe = P(A=s)P(B=s) + P(A=t)P(B=t)
- Variants of Pe formula: Pe independent of A,B; which are replaced by their average
- >2 annotators: multi kappa measure (based on agreement on pairs)
- weighting each disagreement type differently: Krippendorf’s alph
