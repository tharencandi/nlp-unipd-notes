# Introduction to Language Models

## Synthesising Representation and Sequence Probability

Having established the foundational principles of distributional semantics and explored various methodologies for text representation, we now transition to the domain of language modelling.

### Recap
Encoding textual units into numerical formats that capture aspects of their meaning and relationships based on their contextual distribution:

- **Elementary Text Representations:**  **BoW**, **TF-IDF**, and **N-grams:**
  - N-Grams extend beyond the limitations of BoW by considering contiguous sequences of _n_ linguistic units. Captures local contextual dependencies and rudimentary word order.
- **Dense Semantic Embeddings:**
  - semantically similar words are positioned proximally in the vector space, facilitating enhanced generalisation and semantic understanding within NLP models.

We now shift our focus from the static representation of linguistic units to the dynamic modelling of the probability of word sequences.

## Language Models Definition

Statistical frameworks designed to assign a probability measure to any given sequence of *linguistic units*: symbols, tokens, and sequences.

**Applications:** Text Generation; Speech Recognition; Machine Translation; Linguistic Error Correction; Information Retrieval.

---

**See [[6 - 2 N-Gram Language Model|N-Gram Language Model]] for elaboration on introductory statistical techniques for estimating sentence probabilities.**
